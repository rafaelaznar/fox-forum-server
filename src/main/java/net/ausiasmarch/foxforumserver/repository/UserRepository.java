...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
  ...
  Page<UserEntity> findByNameContaining(String name, Pageable pageable);
    ...
    @Query(value = "SELECT * FROM user WHERE length(?1) >= 5 AND (name LIKE %?1% OR surname LIKE %?1% OR lastname LIKE %?1% OR username LIKE %?1% OR email LIKE %?1%)", nativeQuery = true)
